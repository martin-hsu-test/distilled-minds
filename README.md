...
除了 persona，本專案還附帶 meta-skill — **安裝任何 persona 時會自動一併安裝**，不會出現在選單裡。

| Skill | 用途 | 呼叫方式 |
|-------|------|--------|
| **ask-more** | 多視角圓桌會議。掃描所有 `perspective-*` skill，平行詢問每個人物對同一問題的觀點，最後以「最高存在」身份綜合判斷，給出最認同的觀點、會做的 5 件事、不會做的 5 件事。 | `/ask-more <你的問題>` — 只透過顯式 slash command 呼叫，不會被自然語言自動觸發 |
